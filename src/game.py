...
class Game:
    ...
    def _setup_main_ui_labels(self):
        ...
        self.items_collected_label = pygame_gui.elements.UILabel(
            relative_rect=pygame.Rect(10, 70, 240, 30), text=f"Hàng đã giao: {self.player.items_collected_count} / {config.TARGET_ITEMS_TO_WIN}",
    ...
    def _check_and_handle_game_over_conditions(self):
        ...
        if self.player.items_collected_count >= config.TARGET_ITEMS_TO_WIN and self.main_game_time_remaining > 0:
            print("Player has won the game!"); self._set_status_message("CONGRATULATIONS, YOU WON!", is_game_over_message=True)
        ...
        if self.main_game_time_remaining <= 0:
            self.main_game_time_remaining = 0; print("Time's up! GAME OVER.")
            self._set_status_message("TIME'S UP! YOU LOST!", is_game_over_message=True)
            self.current_game_state = config.ST_GAME_OVER_TIME; self.game_over_message_shown = True
            self._cleanup_active_minigame_and_ui(); return True
        
        if self.player.money <= 0:
            print("Out of money! GAME OVER."); 
            self._set_status_message("YOU RAN OUT OF MONEY! GAME OVER!", is_game_over_message=True)
    ...
    def _update_game_logic(self, time_delta):
        ...
        if self.items_collected_label.alive(): self.items_collected_label.set_text(f"Hàng đã giao: {self.player.items_collected_count}/ {config.TARGET_ITEMS_TO_WIN}" )
    ...
    def _draw_game_elements(self):
        ...
        if self.current_game_state == config.ST_PLAYING_PUZZLE and self.current_minigame_instance and self.current_minigame_instance.is_active:
            ...
            game_end_font = pygame.font.Font(config.DEFAULT_FONT_PATH, 60)
            message = ""
            color = config.RED
            ...
            if message:
                ...
                play_again_font = pygame.font.Font(config.DEFAULT_FONT_PATH, 36)
